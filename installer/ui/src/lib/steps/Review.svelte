<script>
  ...
  onMount(async () => {
    try {
      ...
      checks = { ready: true }; // Don't block on preflight failure
    }
    checking = false;
  });
...
<div class="max-w-3xl mx-auto animate-fade-in">
  ...
  <div class="space-y-4">
    <!-- Preflight checks -->
    {#if checks && !checks.ready}
      <Alert variant="error">
        <div class="space-y-2">
          <p class="font-medium">Prerequisites not met:</p>
          {#if checks.docker && !checks.docker.ok}
            <div class="flex items-center gap-2">
              <svg class="w-4 h-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
              </svg>
              <span>{checks.docker.error}</span>
            </div>
          {/if}
          <button
            type="button"
    ...
    {:else if checks?.ready}
      <Alert variant="success">
        <div class="flex items-center gap-2">
          <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
          </svg>
          <span>All prerequisites met — ready to install.</span>
        </div>
      </Alert>
    {/if}
      ...
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-xs text-gray-500 dark:text-gray-400">
            {#if state.disableAI}
              Deterministic checks + whitelist only
            {:else if state.llm?.provider === 'local'}
  ...
  <div class="flex justify-between mt-8">
    ...
    <Button variant="success" size="lg" onclick={handleInstall} disabled={checking || (checks && !checks.ready)}>
